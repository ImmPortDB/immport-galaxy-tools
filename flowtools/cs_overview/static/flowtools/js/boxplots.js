var displayMarkerTable = function(plotconfig){
    ...
    plotconfig.allMarkers.map(function(v) {
        ...
        + '<td align="center"><input type="checkbox" checked class=' + plotconfig.mrkrSelect
        + ' value=' + v + '/></td></tr>');
    });
...
var updateBoxplot = function(plotconfig){
    ...
    var w = $(plotconfig.plotdivj).width(),
    ...
    var labels = false, // show the text labels beside individual boxplots?
        mfi_option = false;
    var checkLabels = $(plotconfig.displayvalues).prop("checked");
    if (checkLabels) {
        labels = true;
    };
    var checkMFI = $(plotconfig.displayMFI).prop("checked");
    if (checkMFI) {
        mfi_option = true;
    };
    var min = Infinity,
    ...
    */
    ...
    if (plotconfig.view == 'p') {
        plotconfig.selectedPopulations.forEach(function(p) {
            ...
            plotconfig.selectedMarkers.forEach(function(m) {
                ...
                var qtmp = [+plotconfig.csdata.q1[markernm][p],
                            +plotconfig.csdata.q2[markernm][p],
                            +plotconfig.csdata.q3[markernm][p]
                          ];
                var wtmp = [+plotconfig.csdata.lower[markernm][p],
    ...
    if (plotconfig.view == 'p'){
        ...
        group.selectAll(".box")
            ...
            .enter().append("g")
            .attr("transform", function(d) {
                return "translate(" + x1Scale(d.marker) + ",0)";
            })
            .call(boxplot);
        ...
        group.selectAll(".box")
            ...
            .enter().append("g")
            .attr("transform", function(d) {
                return "translate(" + x1Scale(d.population) + ",0)";
            })
            .call(boxplot);
    }
};
                ...
                if (showMFI == true) {
                    MFILine.enter().append("line")
                        ...
                        .style("stroke", function(d){
                            return so_palette[data.config[1]] + '255)';
                        })
                        .attr("x1", 0)
                        .attr("y1", x0)
                        .attr("x2", width)
